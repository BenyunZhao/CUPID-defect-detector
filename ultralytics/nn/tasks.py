...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, DetectAux, Segment)):
...
def attempt_load_one_weight(weight, device=None, inplace=True, fuse=False):
    ...
    for m in model.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Segment, Pose, OBB, DetectAux):
            m.inplace = inplace
        elif t is nn.Upsample and not hasattr(m, "recompute_scale_factor"):
            m.recompute_scale_factor = None  # torch 1.11.0 compatibility
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        elif m in (Detect, DetectAux, Segment, OBB):
            args.append([ch[x] for x in f])
            if m is Segment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
        elif m is RTDETRDecoder:  # special case, channels arg must be passed in index 1
            args.insert(1, [ch[x] for x in f])
        elif m is MSI_Cache_Layer:
        ...
        elif m is MSI_Fuse_Layer:
            c2 = ch[f[-1]]
        else:
            c2 = ch[f]

        if isinstance(c2, list):  # Handle backbone case
            is_backbone = True
            m_ = m
            m_.backbone = True
        else:
...
def guess_model_task(model):
    ...
    def cfg2task(cfg):
        ...
        if m in ("classify", "classifier", "cls", "fc"):
            return "classify"
        if "detect" in m:  # 修改这里,使其能识别 detect 相关的模块
            return "detect"
        if m == "segment":
            return "segment"
        if m == "pose":
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Detect, DetectAux)):
                return "detect"
